...
class RedisVectorSearch:
    ...
    def _get_profile_text(self, hash_key):
        ...
        Incluye nombre_completo y bio además de los campos originales.
        """
        profile = self.redis_client.hgetall(hash_key)
        if not profile:
            return None

        # Construye un texto que facilite búsquedas específicas incluyendo los nuevos campos
        return (
            f"Red social: {hash_key.split(':')[0]} | "
            f"Nombre Completo: {profile.get('nombre_completo', '')} | "
    ...
    def create_index(self, force=False):
        ...
        if self.redis_client.exists(self.index_name) and not force:
            print(
                f"El índice {self.index_name} ya existe. Use force=True para recrearlo."
            )
            return None
        ...
        # Obtener todos los IDs de todas las redes sociales
        ...
        for emily_id in all_ids:
            ...
            if profile_texts:
                # Crear documento con todos los perfiles de redes sociales de nuestra emily en turno
                metadata = {"emily_id": emily_id, "text_type": "profile"}
...
if __name__ == "__main__":
    ...
    queries = [
        "I want a flexible woman who knows how to prepare alcoholic drinks and do a lot of exercise",
    ]
